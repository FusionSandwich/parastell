...
Therefore the general ParaStell + stock OpenMC 0.16.0 baseline must model **one full 90-degree period** and use ordinary rotational periodic planes at 0 and 90 degrees. The 452 independent half-period thickness values are expanded to a 31 x 31 full-period matrix using ParaStell helical symmetry. The stored `source_mesh.h5m` was also generated by `stellarator_optimization` over a full period, but its domain, element order, strengths, and normalization still require a fresh hash-bound audit before reuse.
