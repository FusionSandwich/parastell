...
This evidence is not yet a completed transport gate. The later repaired radial
PLC matrix at 13 x 49, 17 x 65, and 21 x 81 resolution passes Gmsh
tetrahedralization and an independent mesh audit for both ported and unported
cases; see [the repair record](ACTUAL_PLC_REPAIR_2026-08-24.md). That matrix
did not include magnets or a full reactor assembly. The external overlap check
and subsequent OpenMC gate must also pass before this model is described as
full reactor transport validation. No full-reactor PNG is emitted while any
of those gates is unresolved.
