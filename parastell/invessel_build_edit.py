...
class InVesselBuild(object):
    ...
    def _get_or_create_vertices(self, coord_list):
        """Helper to get or create a list of vertex handles for the
        input list of 3D coordinates, avoiding duplicates.
        """
        vert_handles = []
        for c in coord_list:
            # Apply rounding to handle floating-point precision
            c_tup = tuple(np.round(c, decimals=7))  # Adjust decimals as needed
            if c_tup in self._vertex_map:
                vert_handles.append(self._vertex_map[c_tup])

            else:
                # Corrected: Use create_vertices and extract the first handle
    ...
    def generate_components_pydagmc(self):
        ...
        # Create volumes explicitly, including the plasma volume
        total_surfaces = len(surface_list)
        for _ in range(total_surfaces + 1):  # Create an extra volume
            dagmc.Volume.create(self.dag_model)

        # Assign surface senses for curved surfaces
        for i, sid in enumerate(curved_surface_ids):
            if i == 0:
                # First surface: plasma (Volume 1) to next (Volume 2)
            ...
            for sid in pair_ids:
                if i + 2 < len(volume_handles):
                    ...
                    self._logger.error(
                        ...
                        f"but tried to access index {i + 2}."
                    )
                    raise IndexError(
                        f"Cannot assign surf_sense for surface ID {sid}. "
                        f"Insufficient volume handles: {len(volume_handles)} available, "
