...
def create_moab_tris_from_corners(corners, mbc, get_or_create_vertices):
    ...
    Arguments:
        ...
        get_or_create_vertices (callable): function returning vertex handles
            for each (x,y,z) in corners.

    Returns:
        list: [tri_1, tri_2] triangular mesh elements
    """
...
class InVesselBuild(object):
    ...
    Optional attributes:
        ...
        num_ribs (int): total number of ribs over which to loft for each build
            segment (defaults to 81). Ribs are set at toroidal angles
        ...
        num_rib_pts (int): total number of points defining each rib spline
            (defaults to 97). Points are set at poloidal angles interpolated
    ...
    def __init__(self, vmec_obj, radial_build, logger=None, **kwargs):
        ...
        self._vertex_map = {}          # Mapping from index to vertex handle
        self._vertex_coords = []       # List of vertex coordinates
        self._vertex_tree = None       # KD-Tree for spatial queries
        self.repeat = 0
        self.num_ribs = kwargs.get("num_ribs", 61)       # Increased from 61 to 81
        self.num_rib_pts = kwargs.get("num_rib_pts", 67) # Increased from 67 to 97
        self.scale = m2cm
    ...
    def _build_vertex_tree(self):
        """Builds a KD-Tree from the current vertex coordinates for efficient querying."""
        if self._vertex_coords:
            self._vertex_tree = cKDTree(self._vertex_coords)
        else:
            self._vertex_tree = None

    def _get_or_create_vertices(self, coord_list, tolerance=1e-5):
        """Helper to get or create a list of vertex handles for the
        input list of 3D coordinates, avoiding duplicates using a KD-Tree.

        Arguments:
            coord_list (list of list/tuple): list of 3D coordinates.
            tolerance (float): distance tolerance for considering vertices identical.

        Returns:
            list: list of vertex handles.
        """
        vert_handles = []
        new_vertices = []
        new_coords = []
        ...
        for c in coord_list:
            c = np.array(c)
            if self._vertex_tree is not None:
                distance, idx = self._vertex_tree.query(c, distance_upper_bound=tolerance)
                if distance < tolerance:
                    vert_handles.append(self._vertex_map[idx])
                else:
                    new_vertices.append(c)
                    new_coords.append(tuple(c))
            else:
                new_vertices.append(c)
                new_coords.append(tuple(c))

        if new_vertices:
            created_vertices = self.mbc.create_vertices(new_vertices)
            for coord, v_handle in zip(new_coords, created_vertices):
                self._vertex_map[len(self._vertex_coords)] = v_handle
                self._vertex_coords.append(coord)
                vert_handles.append(v_handle)
            self._build_vertex_tree()  # Rebuild the KD-Tree after adding new vertices
    ...
    def connect_ribs_with_tris_moab(self, rib1, rib2, reverse=False):
        """Connects two ribs by creating triangular elements between them."""
        mb_tris = []
        for rib_loci_index in range(len(rib1.rib_loci) - 1):
            ...
            if reverse:
                mb_tris += create_moab_tris_from_corners(
                    corners[::-1], self.mbc, self._get_or_create_vertices
                )
            else:
                mb_tris += create_moab_tris_from_corners(
                    corners, self.mbc, self._get_or_create_vertices
                )
        return mb_tris

    def generate_components_pydagmc(self):
        """Generates all components in the DAGMC model with proper vertex handling."""
        self._logger.info("Generating DAGMC surfaces and volumes...")
        ...
        for i, surface in enumerate(self.Surfaces.values()):
            if i == 0:
                surface.generate_pydagmc_surface(
                    self.dag_model,
                    self.mbc,
                    self._get_or_create_vertices,  # Pass the callable here
                    reverse=True
                )
            else:
                surface.generate_pydagmc_surface(
                    ...
                    self.mbc,
                    self._get_or_create_vertices,  # Pass the callable here
                    reverse=False  # Explicitly pass reverse=False
                )
        curved_surface_ids = list(self.dag_model.surfaces_by_id.keys())

        # Generate the end cap surfaces between consecutive surfaces
        end_cap_surface_ids = []
        surface_list = list(self.Surfaces.values())

        for s_idx in range(len(surface_list) - 1):
            current_surface = surface_list[s_idx]
            next_surface = surface_list[s_idx + 1]

            # Start side
            mb_tris = self.connect_ribs_with_tris_moab(
                current_surface.Ribs[0], next_surface.Ribs[0]
            )
        ...
        # Explicitly create the plasma chamber volume
        plasma_chamber_volume = dagmc.Volume.create(self.dag_model)

        # Create additional volumes
        total_surfaces = len(surface_list)
        for _ in range(total_surfaces):
            dagmc.Volume.create(self.dag_model)

        # Assign surface senses for curved surfaces
        for i, sid in enumerate(curved_surface_ids):
            if i == 0:
                # First surface: plasma chamber (plasma_chamber_volume) to next volume
                self.dag_model.surfaces_by_id[sid].surf_sense = [
                    plasma_chamber_volume,
                    self.dag_model.volumes_by_id[i + 1],
                ]
            elif i != total_surfaces - 1:
                # Middle surfaces: Volume i to Volume i +1
                self.dag_model.surfaces_by_id[sid].surf_sense = [
        ...
        # Assign senses to end caps with safety checks
        volume_handles = list(self.dag_model.volumes_by_id.values())

        for i, pair_ids in enumerate(end_cap_surface_ids):
            for sid in pair_ids:
                # Adjust indexing since plasma chamber is at index 0
                adjusted_i = i + 1  # plasma_chamber_volume is volume_handles[0]
                if adjusted_i + 1 < len(volume_handles):
                    self.dag_model.surfaces_by_id[sid].surf_sense = [
                        volume_handles[adjusted_i],
                        volume_handles[adjusted_i + 1],
                    ]
                else:
                    self._logger.error(
                        f"Cannot assign surf_sense for surface ID {sid}. "
                        f"Insufficient volume handles: {len(volume_handles)} available, "
                        f"but tried to access index {adjusted_i + 1}."
                    )
                    raise IndexError(
                        f"Cannot assign surf_sense for surface ID {sid}. "
                        f"Insufficient volume handles: {len(volume_handles)} available, "
...
class Surface(object):
    ...
    def generate_pydagmc_surface(self, dag_model, mbc, get_or_create_vertices, reverse=False):
        """Generates a DAGMC surface using MOAB triangles.

        Arguments:
            dag_model (dagmc.DAGModel): The DAGMC model to add surfaces to.
            mbc (pymoab.Core): The MOAB core instance.
            get_or_create_vertices (callable): Function to get/create vertex handles.
            reverse (bool): Whether to reverse the winding order.
        """
        mb_tris = []
        for rib, next_rib in zip(self.Ribs[0:-1], self.Ribs[1:]):
            for rib_pt_index in range(len(rib.rib_loci) - 1):
                ...
                corners = [corner1, corner2, corner3, corner4]
                if reverse:
                    # Correctly pass get_or_create_vertices
                    mb_tris += create_moab_tris_from_corners(
                        corners[::-1], mbc, get_or_create_vertices
                    )
                else:
                    mb_tris += create_moab_tris_from_corners(
                        corners, mbc, get_or_create_vertices
                    )
...
class RadialBuild(object):
    ...
    Arguments:
        ...
        split_chamber (bool): if wall_s > 1.0, separate interior vacuum
            ...
            radial_build dictionary, setting this to False will not combine
