    ...
    *,
    ...
    for name, value in optional_columns.items():
        if value is not None:
            ...
            columns[name] = array
    if openmc_weight is not None:
        values = np.asarray(openmc_weight, dtype=float)
        if (
            values.shape != (n,)
            ...
            raise ValueError(
                "openmc_weight must be finite, nonnegative, and align with records"
            )
        columns["openmc_weight"] = values
